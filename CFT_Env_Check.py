# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 20:24:36 2023
...
from Prepared_validate_data import Discretize_validate_data
import numpy as np


class CFT_Env_Check(gym.Env):
    metadata = {'render.modes': ['console']}

    def __init__(self,
                 scale, step_size, ep_steps, episodes, neurons, nx=20
                 ):
        super(CFT_Env_Check, self).__init__()
        print(f'Neurons:{neurons}_Eval start')
        ...
        self.err_analyitc_list = ising_viol_30()
    ...
    def step(self, action):
        ...
        if self.step_num == self.ep_steps:
            ...
            with open(f'Rewards_Neuron:{self.neurons}_Check.csv', 'ab') as file:
                np.savetxt(file, self.Reward_collector)
                
            file = open(f'Best_with_maxreward_Neuron:{self.neurons}_Check.csv',mode='a')
    ...
    def reset(self):
        ...
        self.delta_theta_list, self.ope_c_list = Discretize_validate_data(self.ep_counting)
        self.delta_list = self.spin_list + (6.5-self.spin_list)*np.sin(self.delta_theta_list*np.pi/2)**2
